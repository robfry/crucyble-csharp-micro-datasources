...
{
  public static class SysmgmtLandesk
  {
    public static FidoReturnValues RunLandeskQuery(FidoReturnValues lFidoReturnValues, string sSrcIP, string hostname)
    {
      var sLandeskSrcIP = String.Empty;
      var lQuery = new List<string>();
      var lHostInfo = new List<string>();

      var landeskConf = SQL_Queries.GetSQLConfigs();
      ...
      if ((landeskConf.rows[0].key.ToLower() == "landesk") && (!String.IsNullOrEmpty(sSrcIP)))
      {
        sLandeskSrcIP = SysmgmtLandesk.FormatIP(sSrcIP);
      }

      lQuery.Add(landeskConf.rows[0].value.configs.connstring);
      lQuery.Add(landeskConf.rows[0].value.configs.query.queryip);
      lQuery.Add(landeskConf.rows[0].value.configs.query.queryhostname);
      ...
      {
        lHostInfo.AddRange(SQL_Queries.RunMSsqlQuery(lQuery, null, hostname));
      }
      ...
      if (lHostInfo[0] != "unknown")
      {
        ...
        {
          lFidoReturnValues.Inventory.Landesk = new LandeskReturnValues();
        }


        lFidoReturnValues = SysmgmtLandesk.GetCBStatus(lFidoReturnValues, lQuery[0]);

        if (lFidoReturnValues.Inventory.Landesk.CBRunning == null)
        {
          lFidoReturnValues.Inventory.Landesk.CBRunning = "Not running";
        }
        if (lFidoReturnValues.Inventory.Landesk.CBVersion == null)
        {
          lFidoReturnValues.Inventory.Landesk.CBVersion = "Not Installed";
        }

        lFidoReturnValues = SysmgmtLandesk.GetSentinelStatus(lFidoReturnValues, lQuery[0]);

        if (lFidoReturnValues.Inventory.Landesk.SentinelRunning == null)
        {
          lFidoReturnValues.Inventory.Landesk.SentinelRunning = "Not running";
        }
        if (lFidoReturnValues.Inventory.Landesk.SentinelVersion == null)
        {
          lFidoReturnValues.Inventory.Landesk.SentinelVersion = "Not Installed";
        }


      }

      if (lFidoReturnValues.Inventory?.Landesk == null) return lFidoReturnValues;

      if (lFidoReturnValues.Inventory.Landesk != null)
      {
        lFidoReturnValues.Inventory.PrimInv = "landesk";
        lFidoReturnValues.Inventory.Domain = lFidoReturnValues.Inventory.Landesk.Domain;
        lFidoReturnValues.Inventory.CBRunning = lFidoReturnValues.Inventory.Landesk.CBRunning;
        lFidoReturnValues.Inventory.CBVersion = lFidoReturnValues.Inventory.Landesk.CBVersion;
        lFidoReturnValues.Inventory.LastUpdated = lFidoReturnValues.Inventory.Landesk.LastUpdate;
        lFidoReturnValues.Inventory.OSName = lFidoReturnValues.Inventory.Landesk.OSName + " " + lFidoReturnValues.Inventory.Landesk.OSVersion;
        lFidoReturnValues.Inventory.SentRunning = lFidoReturnValues.Inventory.Landesk.SentinelRunning;
        lFidoReturnValues.Inventory.SentVersion = lFidoReturnValues.Inventory.Landesk.SentinelVersion;
      }

      return lFidoReturnValues;
    }

    private static FidoReturnValues GetCBStatus(FidoReturnValues lFidoReturnValues, string sConnectionString)
    {
    ...
    private static FidoReturnValues GetSentinelStatus(FidoReturnValues lFidoReturnValues, string sConnectionString)
    {
      //todo: move this to the DB
    ...
    private static string FormatIP(string sIP)
    {
